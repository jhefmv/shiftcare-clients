...
RSpec.describe SearchClient::Duplicate do
  describe '.call' do
    ...
    let(:file_not_found) { 'JSON file cannot be found!' }
    let(:file_cannot_be_parsed) { 'JSON file cannot be parsed!' }

    it 'returns empty results' do
      results = described_class.call(field: 'name')
      expect(results).to be_empty
    end

    it 'returns duplicate entries' do
      results = described_class.call(field: 'email')
      expect(results.keys).to contain_exactly(client_jane[:email])
      expect(results.values[0].map { |r| r.transform_keys(&:to_sym) }).to contain_exactly(client_jane, client_jane2)
    end

    it 'returns duplicate entries from provided JSON file' do
      results = described_class.call(field: 'email', file_path: duplicates_json)
    ...
    it 'returns error when JSON file is not found' do
      expect { described_class.call(field: 'email', file_path: 'missing.json') }.to raise_error(file_not_found)
    end

    it 'returns error when JSON file is invalid' do
      expect { described_class.call(field: 'email', file_path: invalid_json) }.to raise_error(file_cannot_be_parsed)
    end

    it 'returns error when field is missing' do
      expect { described_class.call(query: 'Smith') }.to raise_error('Missing field!')
    end
  end
end